...
func TestGetHandler(t *testing.T) {
	...
	cases := []testCase{
		testCase{handlerCtx, map[string]string{"networkId": "testnetwork", "subnetName": "testsubnet"}, map[string]string{}, `{"requestor":"testnode"}`, &IpamRangeResponse{StartIP: net.ParseIP("2001:db8::601c:e1fa:0:0"), EndIP: net.ParseIP("2001:db8::601c:e1fa:ffff:ffff"), Mask: 64, Gateway: net.ParseIP("2001:db8::1")}, http.StatusCreated},
	...
	for _, c := range cases {
		...
		case *IpamRangeResponse:
			body := &IpamRangeResponse{}
			err = json.Unmarshal([]byte(response.Body), body)
			if err != nil {
